set input {[1518-04-25 00:24] falls asleep
...
foreach line [split $input "\n"] {
	...
	set date [lrange $line 0 1]
	set action [lrange $line 2 end]
	lappend data [list [clock scan $date] [scan [lindex [split $date \:] end] %d] $action]
}

set data [lsort -index 0 -integer -increasing $data]


foreach line $data {
	
	set isguard [regexp -inline -- {[0-9]+} [lrange $line 2 end]]
	if {$isguard!=""}  {
			set gn $isguard;
	} else {
		
		if {[lindex [lindex $line 2] 0] eq "falls"} {
			set sleep [lindex $line 1]
		}
		
		if {[lindex [lindex $line 2] 0] eq "wakes"} {
			for {set i $sleep} {$i<[lindex $line 1]} {incr i} {
				incr guardsleep($gn,$i)
				incr guardsmin($gn)
			}
		}
	}
...
set maxguard [lindex [lindex [lsort -decreasing -index 1 -stride 2 -integer [array get guardsmin]] 0] 0]

set result [lsort -stride 3 -index 2 -decreasing -integer [regsub -all {\,} [array get guardsleep "$maxguard*"] " "]]

puts [expr [lindex $result 0]  * [lindex $result 1]]