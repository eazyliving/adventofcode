set input {dcga cadgbfe gecba cbfde eda cdbea gbadfe fegcba bedgca da | bgefdac bdace ad agcd
...
set input1 {be cfbegad cbdgef fgaecd cgeb fdcge agebfd fecdb fabcd edb | fdgacbe cefdb cefbgd gcbe
...
set segcount {6 2 5 5 4 5 6 3 7 6} 
#set segcount {2 4 3 7}
#set segnums {1 4 7 8}
foreach line [split $input "\n"] {
	lappend signals [lindex [split $line |] 0]
	lappend outputs [lindex [split $line |] 1]
} 
set count 0
foreach out [ join $outputs] {

	set possible [lsearch -all $segcount [string length $out]]
	puts $possible
	if {$possible==-1} {continue}
	if {[llength $possible]==1} {
		puts "$out $possible"
		lappend guess($possible) $out
	}
	incr count
}

puts [array get guess]