...
proc byr val {
	return [expr 1920<=$val && $val<=2002]
}
proc iyr val {
	return [expr 2010<=$val && $val<=2020]
}
proc eyr val {
	return [expr 2020<=$val && $val<=2030]
}
proc hgt val {
	set unit [string range $val end-1 end]
	if {[lsearch "in cm" $unit]==-1} {return 0}
	set val [string range $val 0 end-2]
	if {$unit=="cm"} {
		return [expr 150<=$val && $val<=193]
	}

	return [expr 59<=$val && $val<=76]

}
proc hcl val {
	if {[string index $val 0]!="#"} {return 0}
	if {[string length $val]!=7} {return 0}
	
	return [string is xdigit [string range $val 1 end]]
}
proc ecl val {
	return [expr [lsearch "amb blu brn gry grn hzl oth" $val]!=-1]
}
proc pid val {
	if {[string length $val]!=9} {return 0}
	return [string is digit $val]
}
proc cid val {return 1}
regsub -all "\n\n" $input "|" output
regsub -all "\n" $output " " output
set input [split $output "|"]

set valid 0

foreach pass $input {
	set reco {byr iyr eyr hgt hcl ecl pid}
	set commands ""
	
	foreach pair $pass {
		
		foreach {key val} [split $pair \:] {break}
		set reco [lreplace $reco [lsearch $reco $key] [lsearch $reco $key]]
		lappend commands [list $key $val]
