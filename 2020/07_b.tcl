set input {posh blue bags contain 5 plaid chartreuse bags, 3 plaid lime bags.
...
proc getBagsInside {index bagname} {

	set count_inside 0
			
	foreach container $index {
	
		if {[lindex $container 0]==$bagname} {
			puts $container
			foreach {baginside count} [lrange $container 1 end] {
			
				incr count_inside $count
				incr count_inside [expr [getBagsInside $index $baginside] * $count]
...
set policies [lrange [split $input \.] 0 end-1]

set comp ""
set search "shiny gold"


foreach policy $policies {

	set policy [split [regsub -all "bags contain" $policy "|"] "|"]
	set container [string trim [lindex $policy 0]]
	if {[string match "*no other bags*" $policy]} {
		continue
	}
	set thisp ""

	foreach contents [split [join [lrange $policy 1 end]] \,] {
		
		set bag [string trim [regsub -all {bag[s]*} $contents ""]]
		
		set count [lindex $bag 0]
		if {$count=="no"} {continue}
		set insert [lrange $bag 1 end]
		lappend thisp $insert $count
	}
	
	append comp "{$container} $thisp|"

}


set policies [lrange [split $comp "|"] 0 end-1]
set bags {shiny gold}
# set bags [list $search]
# set bags [join $argv]

puts [getBagsInside $policies $bags]

# do {
# 	puts "searching for $bags"
# 	set found ""
# 	foreach bag $bags {
# 		
# 		foreach line $policies {
# 			if {[lsearch $line $bag]==0} {
# 				
# 			
# 				lappend found [lindex $line 0]
# 				lappend containers [lindex $line 0]	
# 				#puts [lindex $line 0]
# 			}
# 		}
# 	
